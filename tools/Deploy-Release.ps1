...
(Get-Content $location) -Replace 'BUILD_MODE = \"(.*?)\"', 'BUILD_MODE = "RELEASE"' | Set-Content $location

Write-Host -ForegroundColor Green -BackgroundColor Black "Switch BUILD_MODE to RELEASE"
...
pscp -r -p -P $env:SERVER_PORT -l $env:SERVER_USER -pw $env:SERVER_PASSWORD "D:\Programming\Forge Projects\Mine Mine no Mi - 1.14.4\build\libs\*.jar" "$env:SERVER_USER@$env:SERVER_DN$env:SERVER_SAVE_PATH/"
