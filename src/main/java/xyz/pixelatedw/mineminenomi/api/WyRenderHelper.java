...
{	
	...
	{
        Minecraft.getInstance().getTextureManager().bindTexture(new ResourceLocation(Env.PROJECT_ID, "textures/abilities/" + WyHelper.getResourceName(iconName) + ".png"));        
		BufferBuilder bufferbuilder = Tessellator.getInstance().getBuffer();
	...
	{
		Minecraft.getInstance().getTextureManager().bindTexture(new ResourceLocation(Env.PROJECT_ID, "textures/items/" + WyHelper.getResourceName(iconName) + ".png"));        
		BufferBuilder bufferbuilder = Tessellator.getInstance().getBuffer();
		bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX);
		bufferbuilder.pos(x			, y + v			, 1).tex(0.0, 1.0).endVertex();
		bufferbuilder.pos(x + u		, y + v			, 1).tex(1.0, 1.0).endVertex();
