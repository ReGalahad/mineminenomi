...
public class FlintlockItem extends Item {
	...
	public ActionResult<ItemStack> onItemRightClick(World world, PlayerEntity player, Hand hand) {
		...
		for (Ability a : data.getEquippedAbilities()) {
			if (a != null) {
				if (a.equals(BreezeBreathBombAbility.INSTANCE) && a.isContinuous()) {
					if (!player.world.isRemote) {
						((ContinuousAbility) a).stopContinuity(player);
		...
		if (player.isSneaking()) {
			...
			if (itemStack.getTag().getBoolean("canUse")) {
				if (itemStack.getTag().getInt("gunPowder") > 0) {
					if ((player.inventory.hasItemStack(new ItemStack(ModItems.BULLET))
							&& itemStack.getTag().getInt("bulletType") == 0)
							|| (player.inventory.hasItemStack(new ItemStack(ModItems.KAIROSEKI_BULLET))
									&& itemStack.getTag().getInt("bulletType") == 1)) {
						...
						for (int i = 0; i < player.inventory.getSizeInventory(); ++i) {
							...
							if ((bulletType == 0 && s.getItem() == ModItems.BULLET)
									|| (bulletType == 1 && s.getItem() == ModItems.KAIROSEKI_BULLET)) {
	...
	@Override
	public void addInformation(ItemStack itemStack, @Nullable World world, List<ITextComponent> list,
			ITooltipFlag par4) {
		if (itemStack.hasTag()) {
